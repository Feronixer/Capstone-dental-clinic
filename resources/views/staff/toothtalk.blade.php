...
<div class="container-fluid" style="max-width: 1200px;">
    ...
    <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('staff-toothtalk.settings.save') }}">
                @csrf
                <div class="row mb-3">
                    <div class="col-md-6">
            ...
            <form id="settingsForm" method="POST" action="{{ route('staff-toothtalk.settings.save') }}" style="display:none;">
                @csrf
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
            ...
            <form id="addFaqForm" method="POST" action="{{ route('staff-toothtalk.faq.store') }}" style="display:none;">
                @csrf
            </form>
                    ...
                    <tbody id="faqTableBody">
                        @forelse($faqs as $faq)
                            <tr data-faq-id="{{ $faq->id }}">
                                <td class="text-muted d-none d-md-table-cell">{{ $loop->iteration }}</td>
                                        <td>
            ...
            @foreach($faqs as $faq)
                                <form id="update-faq-{{ $faq->id }}" method="POST" action="{{ route('staff-toothtalk.faq.update', $faq->id) }}" style="display:none;">
                                    @csrf
                                    @method('PUT')
                                </form>
                                <form id="delete-faq-{{ $faq->id }}" method="POST" action="{{ route('staff-toothtalk.faq.delete', $faq->id) }}" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            @endforeach
...
<script>
        ...
        addBtn.addEventListener('click', () => {
            ...
            if (deleteBtn) {
                deleteBtn.addEventListener('click', async function() {
                    // Get intent label for confirmation message
                    const labelInput = div.querySelector('input[name*="[label]"]');
                    const intentLabel = labelInput ? labelInput.value || 'this intent' : 'this intent';
                    
                    // Show confirmation modal
                    const confirmed = await showConfirmModal(`Are you sure you want to delete "${intentLabel}"?`, {
                        title: 'Delete Intent',
                        icon: 'trash',
                        type: 'danger',
                        okText: 'Yes, Delete'
    ...
    document.getElementById('confirmSaveSettingsBtn').addEventListener('click', function() {
        ...
        const visibleForm = document.querySelector('form[action="{{ route('staff-toothtalk.settings.save') }}"]');
    ...
    function addNewFaqRow() {
        ...
        // Renumber all rows after adding new row
        renumberTableRows();
    ...
    function saveNewFaq(button, tempId) {
        ...
        if (!question || !answer) {
            showWarningModal('Please fill in both question and answer fields.');
            return;
        }
        ...
        // Submit via AJAX
        fetch('{{ route("staff-toothtalk.faq.store") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                showErrorModal('Error: ' + (data.message || 'Failed to save FAQ'));
        ...
        .catch(error => {
            ...
            showErrorModal('An error occurred while saving the FAQ. Please try again.');
            button.disabled = false;
            button.innerHTML = '<i class="bi bi-check"></i>';
        });
    }
    ...
    document.getElementById('confirmDeleteFaqBtn').addEventListener('click', function() {
        ...
        if (deleteForm) {
            ...
            form.method = 'POST';
            form.action = `/staff/toothtalk/faq/${faqToDelete}`;
