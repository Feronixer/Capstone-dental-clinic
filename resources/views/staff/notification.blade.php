...
/* Approve modal header - green gradient */
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                if (data.appointment && data.appointment.month && data.appointment.year) {
                    ...
                    window.location.href = `/staff/appointment?month=${data.appointment.month}&year=${data.appointment.year}&_t=${timestamp}`;
                } else {
                    // Fallback: redirect to current month
                    const timestamp = new Date().getTime();
                    window.location.href = `/staff/appointment?_t=${timestamp}`;
                }
            }, 2000);
        } else {
...
document.getElementById('confirmDenyBtn').addEventListener('click', function() {
    ...
    fetch(`/staff/notifications/deny/${currentRequestId}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
