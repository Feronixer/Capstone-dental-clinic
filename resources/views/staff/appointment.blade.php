        ...
        <div class="modal-content">
            ...
            <form id="blockTimeForm">
                <div class="modal-body">
                    ...
                    <!-- Multi-Day Calendar Picker (Hidden by default) -->
                    <div class="mb-3" id="multi_day_section" style="display: none;">
                        <label class="form-label">SELECT DATE RANGE</label>
                        <div class="date-picker-container">
                            <div class="calendar-widget" id="block-calendar-widget">
                                <div class="calendar-header">
                                    <button type="button" class="btn btn-sm btn-outline-secondary" id="block-prev-month">
                                        <i class="bi bi-chevron-left"></i>
                                    </button>
                                    <span id="block-current-month-year">October 2025</span>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" id="block-next-month">
                                        <i class="bi bi-chevron-right"></i>
                                    </button>
                                </div>
                                <div class="calendar-grid" id="block-modal-calendar">
                                    <!-- Calendar will be generated by JavaScript -->
                                </div>
                            </div>
                        </div>
                        <input type="hidden" id="block_selected_start_date" name="start_date">
                        <input type="hidden" id="block_selected_end_date" name="end_date">
                        <div class="form-text text-muted">
                            <i class="bi bi-info-circle me-1"></i>Click and drag to select a range of days. The clinic will be marked as closed for the entire day(s).
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Block Multiple Days functionality
    let blockCalendarCurrentDate = new Date();
    let isBlockDragging = false;
    let blockSelectionStart = null;
    let blockSelectionEnd = null;
    ...
    document.getElementById('block_multiple_days').addEventListener('change', function(e) {
        ...
        if (isMultiple) {
            ...
            // Generate calendar when switching to multi-day mode
            if (!document.getElementById('block-modal-calendar').hasChildNodes()) {
            ...
            // Clear multi-day selection
            blockSelectionStart = null;
            blockSelectionEnd = null;
            document.getElementById('block_selected_start_date').value = '';
            document.getElementById('block_selected_end_date').value = '';
        }
    });

    // Block calendar navigation
    document.getElementById('block-prev-month')?.addEventListener('click', function() {
        blockCalendarCurrentDate.setMonth(blockCalendarCurrentDate.getMonth() - 1);
        generateBlockCalendar();
    });

    document.getElementById('block-next-month')?.addEventListener('click', function() {
        blockCalendarCurrentDate.setMonth(blockCalendarCurrentDate.getMonth() + 1);
        generateBlockCalendar();
    });

    // Generate block calendar
    function generateBlockCalendar() {
        const calendarGrid = document.getElementById('block-modal-calendar');
        ...
        const firstDay = new Date(blockCalendarCurrentDate.getFullYear(), blockCalendarCurrentDate.getMonth(), 1);
        const lastDay = new Date(blockCalendarCurrentDate.getFullYear(), blockCalendarCurrentDate.getMonth() + 1, 0);
        ...
        for (let i = 0; i < 42; i++) {
            ...
            if (dayNumber >= 1 && dayNumber <= daysInMonth) {
                const cellDate = new Date(blockCalendarCurrentDate.getFullYear(), blockCalendarCurrentDate.getMonth(), dayNumber);
                dayElement.textContent = dayNumber;
                dayElement.dataset.date = `${blockCalendarCurrentDate.getFullYear()}-${String(blockCalendarCurrentDate.getMonth() + 1).padStart(2, '0')}-${String(dayNumber).padStart(2, '0')}`;
                ...
                if (appointmentDate < phToday) {
                    ...
                    dayElement.addEventListener('mousedown', function(e) {
                        if (appointmentDate >= phToday) {
                            ...
                            blockSelectionStart = this.dataset.date;
                            blockSelectionEnd = this.dataset.date;
                    ...
                    dayElement.addEventListener('mouseenter', function() {
                        if (isBlockDragging && appointmentDate >= phToday) {
                            blockSelectionEnd = this.dataset.date;
                            updateBlockSelection();
                        }
                    });
        ...
        const monthYearElement = document.getElementById('block-current-month-year');
        if (monthYearElement) {
            monthYearElement.textContent = blockCalendarCurrentDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        }
    }
    ...
    function updateBlockSelection() {
        if (!blockSelectionStart || !blockSelectionEnd) return;

        // Parse dates
        const startDate = new Date(blockSelectionStart);
        const endDate = new Date(blockSelectionEnd);

        // Sort dates
        let rangeStart = startDate < endDate ? startDate : endDate;
        let rangeEnd = startDate > endDate ? startDate : endDate;

        // Update all calendar days
        document.querySelectorAll('#block-modal-calendar .calendar-day').forEach(day => {
            ...
            day.classList.remove('range-start', 'range-middle', 'range-end', 'range-single');

            if (cellDate.getTime() === rangeStart.getTime() && cellDate.getTime() === rangeEnd.getTime()) {
                day.classList.add('range-single');
            } else if (cellDate.getTime() === rangeStart.getTime()) {
                day.classList.add('range-start');
            } else if (cellDate.getTime() === rangeEnd.getTime()) {
        ...
        document.getElementById('block_selected_start_date').value = blockSelectionStart;
        document.getElementById('block_selected_end_date').value = blockSelectionEnd;
    }
    ...
    function saveBlockTime() {
        ...
        console.log('Block time save - Multiple days:', isMultipleDays);

        if (isMultipleDays) {
            // Multi-day blocking (automatic whole day - 00:00 to 23:59)
            const startDate = document.getElementById('block_selected_start_date').value;
            const endDate = document.getElementById('block_selected_end_date').value;

            if (!startDate || !endDate) {
                showValidationMessage('Please select a date range', 'error');
                return;
            }
            ...
            Promise.all(createPromises)
                .then(results => {
                    ...
                    if (successCount > 0) {
                        ...
                        showValidationMessage(`${successCount} day(s) blocked successfully${failCount > 0 ? ` (${failCount} failed)` : ''}!`, failCount > 0 ? 'warning' : 'success');

                        setTimeout(() => {
                            reloadWithCurrentMonth();
                        }, 1500);
                    } else {
                        showValidationMessage('Failed to block any days', 'error');
                    }
                })
                .catch(error => {
