...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePatientSearchSelection(items, index) {
        ...
        items.forEach((item, i) => {
            ...
            item.classList.remove('patient-result-selected');
            item.style.backgroundColor = '';
            item.style.cursor = 'default';
            
            if (i === index) {
                // Add selected class and style
                item.classList.add('patient-result-selected');
                item.style.backgroundColor = isDarkMode ? '#1e40af' : '#e7f1ff';
    ...
    function generateCalendar() {
        ...
        // Hide all views immediately without transition
    ...
    function saveAppointment() {
        ...
        // Make the request immediately without delay
    ...
    function setupClearBlockOffTimeButton() {
        ...
        newBtn.addEventListener('click', function(e) {
            ...
            fetch('/staff/blocked-time/future/block-off-time/dates', {
                method: 'GET',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Accept': 'application/json',
    ...
    document.getElementById('final-confirm-clear-block-off-time-btn').addEventListener('click', function() {
        ...
        fetch('/staff/blocked-time/future/block-off-time/clear-specific', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Accept': 'application/json',
    ...
    function saveReschedule() {
        ...
        const timeSlotBtn = document.querySelector('#reschedule-time-slots-list .time-slot-btn.selected');
...
/* Selected patient item (when using arrow keys) */
#appointmentModal .patient-result-item.patient-result-selected {
    background-color: #e7f1ff !important;
}

[data-theme="dark"] #appointmentModal .patient-result-item.patient-result-selected {
    background-color: #1e40af !important;
    color: #ffffff !important;
}

[data-theme="dark"] #appointmentModal .patient-result-item.patient-result-selected .patient-name {
    color: #ffffff !important;
}

[data-theme="dark"] #appointmentModal .patient-result-item.patient-result-selected .patient-details {
    color: #dbeafe !important;
}

[data-theme="dark"] #appointmentModal .patient-result-item.patient-result-selected .patient-details .text-muted {
    color: #dbeafe !important;
}
