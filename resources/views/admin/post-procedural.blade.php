                    ...
                    <table class="table table-hover align-middle post-procedural-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" style="width: 14%; min-width: 90px;">Action</th>
                            </tr>
                        </thead>
...
async function buildTreatmentToAppointmentMap() {
    ...
    for (const userId of userIds) {
        try {
            ...
            const recordResponse = await fetch(`/admin/post-procedural/patient-record-by-user/${userId}`);
            ...
            if (recordData.success && recordData.data) {
                ...
                try {
                    const notesResponse = await fetch(`/admin/post-procedural/progress-notes/${recordId}`);
...
async function showDeleteFormSelectionModal(recordId) {
    return new Promise(async (resolve) => {
        ...
        try {
            ...
            const recordResponse = await fetch(`/admin/post-procedural/patient-record/${recordId}`);
            const recordData = await recordResponse.json();
...
async function removeRecordDirect(recordId, formsToDelete = null) {
    ...
    selectedForms.forEach(form => {
        ...
        if (form === 'patient_record') {
            url = `/admin/post-procedural/patient-record/${recordId}`;
            type = 'Patient Record';
        } else if (form === 'patient_history') {
            url = `/admin/post-procedural/patient-history/${recordId}`;
            type = 'Patient History';
        } else if (form === 'progress_notes') {
            const progressNotesDetail = formDetails.progress_notes;
            if (progressNotesDetail && progressNotesDetail.appointment_id) {
                // Delete specific procedure's progress notes
                url = `/admin/post-procedural/progress-notes/${recordId}?appointment_id=${progressNotesDetail.appointment_id}`;
                type = 'Progress Notes (Specific Procedure)';
            } else {
                // Delete all progress notes
                url = `/admin/post-procedural/progress-notes/${recordId}`;
...
function fetchProgressNotesForAppointment(recordId, appointmentId) {
    ...
    fetch(`/admin/post-procedural/progress-notes/${recordId}?appointment_id=${appointmentId}`)
                    .then(response => response.json())
                    .then(notesData => {
...
function openEditNotesModalDirect(recordId, appointmentId = null, treatment = null) {
    ...
    let notesUrl = `/admin/post-procedural/progress-notes/${recordId}`;
    let finalAppointmentId = passedAppointmentId;
    
    if (finalAppointmentId) {
        notesUrl = `/admin/post-procedural/progress-notes/${recordId}?appointment_id=${finalAppointmentId}`;
